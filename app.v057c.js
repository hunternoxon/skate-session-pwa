...
const els={title:$('titleText'),dbgBtn:$('dbgBtn'),settingsBtn:$('settingsBtn'),levelPill:$('levelPill'),levelVal:$('levelVal'),levelDesc:$('levelDesc'),editLevel:$('editLevel'),
...
els.title.addEventListener('click',()=>location.reload()); if(els.dbgBtn){ els.dbgBtn.addEventListener('click',()=>alert('JS live (057c)')); }
...
function setNextVisible(on){ els.nextBtn.classList.toggle('hidden', !on); [els.skipBtn, els.missBtn, els.landBtn].forEach(b=> b.classList.toggle('is-hidden', on)); }
...
function pickStanceBySettings(){const pool=stancePool();if(!pool.length)return null;const nonReg=pool.filter(s=>s!=="regular");if(!STATE.stances.regular&&nonReg.length){return nonReg[Math.floor(Math.random()*nonReg.length)]}if(pool.length===1){return pool[0]}if(nonReg.length&&Math.random()<0.5){return nonReg[Math.floor(Math.random()*nonReg.length)]}return null;}

function generateTrick(){
  const ob=pickObstacle(); 
  const pools={spins:allowedSpins(),flips:allowedFlips(),grinds:allowedGrinds()};
  const base={obstacle:ob};
  const st=pickStanceBySettings(); if(st) base.stance=st;
  ...
  for(let i=0;i<12;i++){
    ...
    let c={...base};
    if(STATE.categories.spins&&pools.spins.length&&Math.random()<0.2){c.spin=pools.spins[Math.floor(Math.random()*pools.spins.length)]}
    if(pat==="flipOnly"){ if(canFlip){ c.flip=pools.flips[Math.floor(Math.random()*pools.flips.length)] } }
    else if(pat==="grindOnly"){ if(canGrind){ const g=pools.grinds[Math.floor(Math.random()*pools.grinds.length)]; c.grind=g; c.direction=RULES.DIRECTION_DEFAULTS[g]||"frontside" } }
    else if(pat==="flipToGrind"){ if(canGrind&&canFlip){ const g=pools.grinds[Math.floor(Math.random()*pools.grinds.length)]; c.grind=g; c.direction=RULES.DIRECTION_DEFAULTS[g]||"frontside"; c.flip=pools.flips[Math.floor(Math.random()*pools.flips.length)] } }
    else if(pat==="manualOnly"){ if(canManual){ c.manual="manual" } }

    // Validate obstacle compatibility
    if(c.grind){
      const slide=["boardslide","noseslide","tailslide","lipslide","bluntslide"].includes(c.grind);
      if(slide && !RULES.SLIDE_OK.has(ob)) c.grind=null;
      if(!slide && !(RULES.GRIND_OK.has(ob)||RULES.SLIDE_OK.has(ob))) c.grind=null;
    }
    if(c.flip && !c.grind && !c.manual){ if(!(RULES.AIR_OK.has(ob)||isRail(ob))) c.flip=null; }

    if(c.flip||c.grind||c.manual){ return c; }
  }

  // Fallbacks
  if(STATE.categories.flips && pools.flips.length){ return {obstacle: RULES.AIR_OK.has(ob)?ob:"flat", flip: pools.flips[0], stance: base.stance} }
  if(STATE.categories.manuals){ return {obstacle: "manual pad", manual: "manual", stance: base.stance} }
  if(STATE.categories.grinds && pools.grinds.length){ const g=pools.grinds[0]; return {obstacle: "rail", grind:g, direction: RULES.DIRECTION_DEFAULTS[g]||"frontside", stance: base.stance} }
  return {obstacle:"flat", flip:"kickflip", stance: base.stance};
}}
function describe(c){
