// Brainlock v0.4.6 — modal fix + CTA sizing + SKATE white-on-miss
const VERSION="0.4.6";

/* ===== State & Defaults (same as v0.4.5) ===== */
const DEFAULTS = {
...
const BASE_POINTS = {
  ...
  flip:{"kickflip":0.3,"heelflip":0.35,"hardflip":0.55,"inward heelflip":0.6,"varial kickflip":0.45,"varial heelflip":0.5,"tre flip":0.8,"360 flip":0.8,"laser flip":0.85,"bigspin flip":0.6,"bigspin heelflip":0.65},
  grind_slide:{"50-50":0.3,"5-0":0.4,"boardslide":0.4,"noseslide":0.45,"tailslide":0.5,"lipslide":0.55,"smith":0.65,"feeble":0.65,"willy":0.55,"salad":0.6,"crooked":0.65,"overcrook":0.75,"nosegrind":0.7,"noseblunt":0.9,"bluntslide":0.85},
  obstacle:{"flat":0,"manual pad":0.1,"box":0.1,"curb":0.15,"ledge":0.25,"hubba":0.35,"flatbar":0.35,"rail":0.45,"handrail":0.55,"gap":0.5,"kicker":0.1,"stair":0.45,"quarterpipe":0.4,"bank":0.25,"funbox":0.2,"mini ramp":0.3},
...
function anyEnabled(obj){ return Object.values(obj).some(v=>!!v); }
function effectiveCategoryEnabled(cat){ if (!STATE.categories[cat]) return false; if (cat==="flips") return anyEnabled(STATE.flips); if (cat==="grinds") return anyEnabled(STATE.grinds); if (cat==="spins") return anyEnabled(STATE.spins); if (cat==="manuals"||cat==="airs") return true; return true; }

/* ===== Generator & validation (unchanged from 0.4.5) ===== */
...
function isValidCombo(c){ const g=c.grind_slide, m=c.manual, ob=c.obstacle, fl=c.flip;
  if (g){ const isSlide=["boardslide","lipslide","tailslide","noseslide","bluntslide"].includes(g);
    if (!ob) return false;
    if (isSlide && !RULES.SLIDE_OK.has(ob)) return false;
    if (!isSlide && !(RULES.GRIND_OK.has(ob) || RULES.SLIDE_OK.has(ob))) return false; }
  if (m){ if (ob && !RULES.MANUAL_OK.has(ob)) return false; }
  if (fl && !g && !m && ob){ if (!(RULES.AIR_OK.has(ob) || isRail(ob))) return false; }
  return true; }
function generateTrick(skill){
  const s=tier(skill), ob=pickObstacle();
  const c={stance:null,spin:null,spin_dir:null,flip:null,grind_slide:null,direction:null,manual:null,obstacle:ob};
  if (Math.random()<(0.10+0.04*s)){ const pool=stancePool(s); if (pool.length){ const st=pool[Math.floor(Math.random()*pool.length)]; c.stance=(st==="regular"?null:st); } }
  const allowFlips=effectiveCategoryEnabled("flips"), allowGrinds=effectiveCategoryEnabled("grinds"), allowSpins=effectiveCategoryEnabled("spins"), allowManuals=effectiveCategoryEnabled("manuals"), allowAirs=effectiveCategoryEnabled("airs");
  if (allowSpins && Math.random()<(0.14+0.05*s)){ const spins=[180,360,540,720].filter(x=>STATE.spins[String(x)]&&allowSpin(x,s)); if (spins.length){ c.spin=spins[Math.floor(Math.random()*spins.length)]; if (Math.random()<0.6) c.spin_dir=(Math.random()<0.5?"frontside":"backside"); } }
  if (allowFlips && Math.random()<(0.24+0.05*s)){ const pool=RULES.FLIPS.filter(n=>STATE.flips[n]&&allowFlip(n,s)); if (pool.length) c.flip=pool[Math.floor(Math.random()*pool.length)]; }
  const canGrindish=(allowGrinds&&(RULES.GRIND_OK.has(ob)||RULES.SLIDE_OK.has(ob)));
  const canManual=(allowManuals&&RULES.MANUAL_OK.has(ob));
  const airish=(allowAirs&&(RULES.AIR_OK.has(ob)||isRail(ob)));
  if (canGrindish && Math.random()<(0.24+0.04*s)){ const pool=RULES.GRINDS_SLIDES.filter(n=>STATE.grinds[n]&&allowGrindSlide(n,s)); if (pool.length){ c.grind_slide=pool[Math.floor(Math.random()*pool.length)]; c.direction=RULES.DIRECTION_DEFAULTS[c.grind_slide]||"frontside"; } }
  else if (canManual && Math.random()<(0.18+0.03*s)){ c.manual="manual"; }
  else if (airish && !c.flip && Math.random()<(0.36+0.05*s)){ const pool=["kickflip","heelflip","varial kickflip"].filter(n=>STATE.flips[n]&&allowFlip(n,s)); if (s>=7 && STATE.flips["tre flip"]) pool.push("tre flip"); if (pool.length) c.flip=pool[Math.floor(Math.random()*pool.length)]; }
  if (!c.flip && !c.grind_slide && !c.manual){ if (airish && allowFlips){ c.flip = STATE.flips["kickflip"] ? "kickflip" : (STATE.flips["heelflip"] ? "heelflip" : null); } if (!c.flip && canGrindish){ c.grind_slide="50-50"; c.direction=RULES.DIRECTION_DEFAULTS["50-50"]; } if (!c.flip && !c.grind_slide && canManual){ c.manual="manual"; } }
  if (!c.flip && !c.grind_slide && !c.manual){ c.obstacle = RULES.AIR_OK.has(ob) ? ob : "flat"; c.flip = "kickflip"; }
  for (let i=0; i<8 && (!isValidCombo(c) || !underScoreCap(c) || !notTooSimilar(c)); i++){ if (!isValidCombo(c)){
      if (c.grind_slide && !(RULES.GRIND_OK.has(c.obstacle)||RULES.SLIDE_OK.has(c.obstacle))){ const opts=[...RULES.GRIND_OK,...RULES.SLIDE_OK].filter(o=>STATE.obstacles.includes(o)); if (opts.length) c.obstacle=opts[Math.floor(Math.random()*opts.length)]; else { c.grind_slide=null; c.direction=null; } }
      else if (c.manual && !RULES.MANUAL_OK.has(c.obstacle)){ const opts=[...RULES.MANUAL_OK].filter(o=>STATE.obstacles.includes(o)); if (opts.length) c.obstacle=opts[Math.floor(Math.random()*opts.length)]; else c.manual=null; }
      else if (c.flip && !c.grind_slide && !c.manual && !(RULES.AIR_OK.has(c.obstacle)||isRail(c.obstacle))){ const opts=[...RULES.AIR_OK].filter(o=>STATE.obstacles.includes(o)); if (opts.length) c.obstacle=opts[Math.floor(Math.random()*opts.length)]; else c.flip=null; }
    } else {
      if (!underScoreCap(c)){ if (c.spin){ c.spin=null; c.spin_dir=null; } else if (c.flip) c.flip=null; else if (c.grind_slide){ c.grind_slide=null; c.direction=null; } }
      if (!notTooSimilar(c)){ if (c.flip && Math.random()<0.5) c.flip=null; if (c.grind_slide && Math.random()<0.5){ c.grind_slide=null; c.direction=null; } if (c.spin && Math.random()<0.5){ c.spin=null; c.spin_dir=null; } }
    } }
  if (!c.spin) c.spin_dir=null; pushRecent(c); return c;
}
function describe(c){
...
const els = {
  ...
  trickBox:document.getElementById('trick'),
  ...
  collapsePreview:document.getElementById('collapsePreview'),
};
...
/* ===== Preview tray collapse ===== */
els.collapsePreview.addEventListener('click', ()=>{
  const collapsed = els.previewTray.classList.toggle('collapsed');
  els.collapsePreview.setAttribute('aria-expanded', String(!collapsed));
});

/* ===== Settings modal open/close ===== */
function openPane(id){
  ["settingsHome","obsPane","stancePane","trickPane","aboutPane"].forEach(pid=>document.getElementById(pid).classList.toggle('active', pid===id));
  els.settingsBack.classList.toggle('hidden', id==="settingsHome");
  els.settingsTitle.textContent = id==="settingsHome" ? "Settings" :
    id==="obsPane" ? "Obstacles" :
    id==="stancePane" ? "Stances" :
    id==="trickPane" ? "Tricks" : "About & Tools";
}
document.getElementById('settingsBtn').addEventListener('click', ()=>{
  openPane("settingsHome"); renderObsChips(); renderStanceChips(); renderCatChips(); renderTrickPanels();
  els.settingsModal.classList.remove('hidden');
});
els.settingsClose.addEventListener('click', ()=> els.settingsModal.classList.add('hidden'));
els.settingsBack.addEventListener('click', ()=> openPane("settingsHome"));
...
els.editObs.addEventListener('click', ()=>{ els.settingsModal.classList.remove('hidden'); openPane("obsPane"); renderObsChips(); });
els.editStance.addEventListener('click', ()=>{ els.settingsModal.classList.remove('hidden'); openPane("stancePane"); renderStanceChips(); });
els.editTricks.addEventListener('click', ()=>{ els.settingsModal.classList.remove('hidden'); openPane("trickPane"); renderCatChips(); renderTrickPanels(); });
...
function renderTrickSummary(){
  const cats=Object.entries(STATE.categories).filter(([k,v])=>v).map(([k])=>k).join("/");
  const enabledFlips=Object.keys(STATE.flips).filter(k=>STATE.flips[k]).length;
  const enabledGrinds=Object.keys(STATE.grinds).filter(k=>STATE.grinds[k]).length;
  const enabledSpins=Object.keys(STATE.spins).filter(k=>STATE.spins[k]).length;
  els.trickSummary.textContent = `${cats||"—"} • F:${enabledFlips} G:${enabledGrinds} S:${enabledSpins}`;
}
function renderAllSummaries(){ renderObsSummary(); renderStanceSummary(); renderTrickSummary(); }
function renderObsChips(){
  const pool=ALL_OBS;
  els.obsChips.innerHTML=pool.map(o=>`<button data-o="${o}" class="${STATE.obstacles.includes(o)?'active':''}">${o}</button>`).join('');
  els.obsChips.querySelectorAll('button').forEach(b=>{
    b.onclick=()=>{ const o=b.dataset.o; STATE.obstacles = STATE.obstacles.includes(o) ? STATE.obstacles.filter(x=>x!==o) : STATE.obstacles.concat([o]); renderObsChips(); renderObsSummary(); saveState(STATE); };
  });
}
function renderStanceChips(){
  const keys=["regular","fakie","nollie","switch"];
  els.stanceChips.innerHTML=keys.map(k=>`<button data-k="${k}" class="${STATE.stances[k]?'active':''}">${k}</button>`).join('');
  els.stanceChips.querySelectorAll('button').forEach(b=>{
    b.onclick=()=>{ const k=b.dataset.k; STATE.stances[k]=!STATE.stances[k]; renderStanceChips(); renderStanceSummary(); saveState(STATE); };
  });
}
function renderCatChips(){
  const map={"flips":"Flips","grinds":"Grinds/Slides","manuals":"Manuals","airs":"Airs","spins":"Spins"};
  els.catChips.innerHTML=Object.keys(map).map(k=>`<button data-k="${k}" class="${STATE.categories[k]?'active':''}">${map[k]}</button>`).join('');
  els.catChips.querySelectorAll('button').forEach(b=>{
    b.onclick=()=>{
      const k=b.dataset.k;
      STATE.categories[k]=!STATE.categories[k];
      if (k==="flips"){ for(const t of Object.keys(STATE.flips)){ STATE.flips[t] = STATE.categories[k] ? STATE.flips[t] : false; } }
      if (k==="grinds"){ for(const t of Object.keys(STATE.grinds)){ STATE.grinds[t] = STATE.categories[k] ? STATE.grinds[t] : false; } }
      if (k==="spins"){ for(const t of Object.keys(STATE.spins)){ STATE.spins[t] = STATE.categories[k] ? STATE.spins[t] : false; } }
      renderCatChips(); renderTrickPanels(); renderTrickSummary(); saveState(STATE);
    };
  });
}
function renderTrickPanels(){
  const flipsDiv=document.querySelector('.panel.flips');
  const grindsDiv=document.querySelector('.panel.grinds');
  const spinsDiv=document.querySelector('.panel.spins');
  flipsDiv.innerHTML=Object.keys(STATE.flips).map(n=>labelBox('flips',n,STATE.flips[n], STATE.categories.flips)).join('');
  grindsDiv.innerHTML=Object.keys(STATE.grinds).map(n=>labelBox('grinds',n,STATE.grinds[n], STATE.categories.grinds)).join('');
  spinsDiv.innerHTML=Object.keys(STATE.spins).map(n=>labelBox('spins',n,STATE.spins[n], STATE.categories.spins)).join('');
  ...
  document.querySelectorAll('#trickPane input[type=checkbox]').forEach(cb=>{
    cb.onchange=()=>{ const g=cb.dataset.group, k=cb.dataset.key; STATE[g][k]=cb.checked; renderTrickSummary(); saveState(STATE); };
  });
}
...
function updateLetters(){
  ...
  spans.forEach((s,i)=>{ s.classList.toggle('lost', i < misses); }); // first N letters lost = white
}
function resetGameState(){
  misses=0; total=0; attempt=1;
  document.getElementById('scoreTop').textContent=total.toFixed(2);
  updateLetters();
}
function startSession(){
  currentSkill=parseInt(els.skill.value,10);
  if (!STATE.obstacles || STATE.obstacles.length===0){ STATE.obstacles = DEFAULTS.obstacles.slice(); }
  saveState(STATE);
  resetGameState();
  setView("game");
  nextTrick(true);
...
function computeScore(c, a){
  ...
  s*=(1+bonus);
  const mult = {1:1.00,2:0.94,3:0.92}[a]||0;
  const base= Math.max(s,0.1);
  return {valid:true, base:round(base,3), combo:round(1+bonus,3), attempt:mult, final:round(base*mult,3), breakdown};
}
...
function updateAttemptUI(){
  ...
  els.attemptPoints.textContent = rep.valid ? `This attempt: ${rep.final.toFixed(2)}` : `This attempt: —`;
}

/* ===== Trick loop ===== */
function nextTrick(first=false){
  attempt=1; updateAttemptUI();
  current = generateTrick(currentSkill);
  for (let i=0; i<3 && (!current || !describe(current)); i++){ current = generateTrick(currentSkill); }
  const text = describe(current) || "kickflip on flat";
  els.trickText.textContent = text;
  els.breakdown.open=false;
  els.nextBtn.classList.add('hidden');
  els.landBtn.disabled=false; els.missBtn.disabled=false; els.skipBtn.disabled=false;
...
function settle(landed){
  if (landed){
    const rep=computeScore(current,attempt);
    total+=rep.final; document.getElementById('scoreTop').textContent=total.toFixed(2);
    els.breakText.textContent=prettyScore(rep);
    els.breakdown.open=true;
    els.nextBtn.classList.remove('hidden');
    els.landBtn.disabled=true; els.missBtn.disabled=true; els.skipBtn.disabled=true;
  } else {
...
function endSession(){
  ...
  const best=Math.max(total, parseFloat(localStorage.getItem('bl_best')||"0"));
  localStorage.setItem('bl_best', String(best));
